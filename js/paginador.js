...
function paginador(actual) {
    ...
    url = location.pathname.replace(reemplazar, "");
    if (actual == 1) {
        pagina--;
    }
    if (actual == 2) {
    ...
    location.href = location.origin + url + 'pagina/' + pagina + location.search;
}
// Cargar Mapa
setTimeout(() => {
    $.ajax({
        url: 'https://www.simi-api.com/ApiSimiweb/response/v2.1.3/filtroInmueble/limite/' + pagina + '/total/12/departamento/0/ciudad/' + ci + '/zona/0/barrio/' + zo + '/tipoInm/' + ti + '/tipOper/' + gs + '/areamin/' + armin + '/areamax/' + armax + '/valmin/' + min + '/valmax/' + max + '/campo/0/order/desc/banios/' + ba + '/alcobas/' + al + '/garajes/' + par + '/sede/' + sede + '',
        type: 'GET',
